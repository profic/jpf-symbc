			...
			if (conditionValue) {
				if (symIntegerValue1 != null) {
					if (symIntegerValue2 != null) { // both are symbolic values
						pathCondition._addDet(Comparator.LE, symIntegerValue1, symIntegerValue2);
					} else {
						pathCondition._addDet(Comparator.LE, symIntegerValue1, integerValue2);
					}
				} else {
					pathCondition._addDet(Comparator.LE, integerValue1, symIntegerValue2);
				}
				...
				if (symIntegerValue1 != null) {
					if (symIntegerValue2 != null) { // both are symbolic values
						pathCondition._addDet(Comparator.GT, symIntegerValue1, symIntegerValue2);
					} else {
						pathCondition._addDet(Comparator.GT, symIntegerValue1, integerValue2);
					}
				} else {
					pathCondition._addDet(Comparator.GT, integerValue1, symIntegerValue2);
				}
				if (!pathCondition.simplify()) {// not satisfiable
					threadInfo.getVM().getSystemState().setIgnored(true);
				} else {

...
public class LADD extends gov.nasa.jpf.jvm.bytecode.LADD {
	...
	public Instruction execute(ThreadInfo threadInfo) {
		...
		if (symLongValue1 == null && symLongValue2 == null) {
			return super.execute(threadInfo); // we'll still do the concrete
												// execution
		} else {
			long longValue1 = stackFrame.popLong();
			long longValue2 = stackFrame.popLong();
			stackFrame.pushLong(0); // for symbolic expressions, the concrete
									// value does not matter

			IntegerExpression symResult = null;
			if (symLongValue1 != null) {
				if (symLongValue2 != null) {
					symResult = symLongValue1._plus(symLongValue2);
				} else {
					symResult = symLongValue1._plus(longValue2);
				}
			} else if (symLongValue2 != null) {
				symResult = symLongValue2._plus(longValue1);

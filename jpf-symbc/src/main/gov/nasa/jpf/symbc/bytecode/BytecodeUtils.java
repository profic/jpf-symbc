...
public class BytecodeUtils {
	 ...
	 * specified in the symbolic.method property Matched based on method name
	 * (short version) & number of params (if any) For methods without
	 * arguments, all methods with this name will be treated as symbolic methods
	...
	public static boolean isMethodSymbolic(Config conf, String methodName, int numberOfArgs, Vector<String> args) {
		...
		if (methods != null) {
			Iterator<String> methodsIterator = Arrays.asList(methods).iterator();
			...
			while (methodsIterator.hasNext()) {
				String method = (String) methodsIterator.next();
				String configMethodName = method.substring(0, method.indexOf("("));
				int argNum;

				if (method.equals(shortName + "()")) {
					argNum = 0;
				} else {
					argNum = method.split("#").length; // number of args
				}
				if (configMethodName.equalsIgnoreCase(shortName)) {
					if (argNum == numberOfArgs) {
						if (args != null) {
							String argString = method.substring(method.indexOf("(") + 1, method.indexOf(")"));
	...
	/*
	 * Uses the class name to determine if a method is symbolic; use to declare
	 * all methods in the given class as symbolic
	 */

	public static boolean isClassSymbolic(Config conf, String className, MethodInfo methodInfo, String methodName) {
		...
		if (classes != null) {
			Iterator<String> classesIterator = Arrays.asList(classes).iterator();

			while (classesIterator.hasNext()) {
				String cName = (String) classesIterator.next();
				if (className.equalsIgnoreCase(cName) && !shortName.equalsIgnoreCase("init") && !methodInfo.isClinit()
	...
	/**
	 * A container for BytecodeUtils.execute to return either the next
	 * instruction to execute or to tell the calling method to call
	 * super.execute();
	 *
	 *
	 */
	public static class InstructionOrSuper {
		...
		/**
		 * Ignore the value of 'inst' and just call the instruction's
	 ...
	 */
	public static InstructionOrSuper execute(JVMInvokeInstruction invInst, ThreadInfo threadInfo) {
		boolean isStatic = (invInst instanceof INVOKESTATIC);
		String bytecodeName = invInst.getMnemonic().toUpperCase();
		String methodName = invInst.getInvokedMethodName();
		String className = invInst.getInvokedMethodClassName();

		MethodInfo methodInfo = invInst.getInvokedMethod(threadInfo);

		if (methodInfo == null) {
			return new InstructionOrSuper(false, threadInfo.createAndThrowException("java.lang.NoSuchMethodException",
					"calling " + className + "." + methodName));
		}

		/*
		 * Here we test if the the method should be executed symbolically. We
		 * perform two checks: 1. Does the invoked method correspond to a method
		 * listed in the symbolic.method property and does the number of
		 * parameters match? 2. Is the method contained in a class that is to be
		 * executed symbolically? If the method is symbolic, initialize the
		 * parameter attributes and the fields if they are specified as symbolic
		 * based on annotations
		 *
		 */

		String longName = methodInfo.getFullName();
		String[] argTypes = methodInfo.getArgumentTypeNames();
		...
		boolean symStringsFlag = (symstrings != null && symstrings[0].equalsIgnoreCase("true")) ? true : false;
		if (symStringsFlag) {
			SymbolicStringHandler symStringHandler = new SymbolicStringHandler();
			Instruction handled = symStringHandler.handleSymbolicStrings(invInst, threadInfo);
		...
		boolean symClass = BytecodeUtils.isClassSymbolic(conf, className, methodInfo, methodName);
		...
		if (found) {
			...
			ChoiceGenerator<?> choiceGenerator = null;
			if (!threadInfo.isFirstStepInsn()) { // first time around
				choiceGenerator = new PCChoiceGenerator(1);
			...
			String outputString = "\n***Execute symbolic " + bytecodeName + ": " + methodName + "  (";
			...
			if (argsInfo != null) {
				localVarsIdx = (isStatic ? 0 : 1); // Skip over "this" argument
													// when non-static
			} else {
			...
			int stackIdx = numStackSlots - 1; // stackIdx ranges from
												// numStackSlots-1 to 0
			...
			for (int j = 0; j < argSize; j++) { // j ranges over actual
												// arguments
				if (symClass || args.get(j).equalsIgnoreCase("SYM")) {
					...
					if (argTypes[j].equalsIgnoreCase("int")) {
						...
						if (symArray) {
							...
							Object[] argValues = invInst.getArgumentValues(threadInfo);
			...
			int index = 1;
			ClassInfo ci = methodInfo.getClassInfo();
			FieldInfo[] fields = ci.getDeclaredInstanceFields();
			ElementInfo ei;
			if (isStatic) {
				ei = threadInfo.getElementInfo(ci.getClassObjectRef());
			} else {
				int objRef = threadInfo.getCalleeThis(invInst.getArgSize());
				if (objRef == MJIEnv.NULL) { // NPE
					return new InstructionOrSuper(false, threadInfo.createAndThrowException(
							"java.lang.NullPointerException", "calling '" + methodName + "' on null object"));
				}
				ei = threadInfo.getElementInfo(objRef);
			}
			...
			if (staticFields.length > 0) {
				for (int i = 0; i < staticFields.length; i++) {
					...
					if (value.equalsIgnoreCase("true")) {
						Expression sym_v = Helper.initializeStaticField(staticFields[i], ci, threadInfo, "");

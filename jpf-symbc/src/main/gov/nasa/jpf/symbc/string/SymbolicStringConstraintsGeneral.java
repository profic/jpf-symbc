 ...
 * 
 * 1. Convert the constraints to a StringGraph (what this class does) 2.
 * Preprocess the StringGraph (gov.nasa.jpf.symbc.string.graph.PreProcessGraph
 * 3. Solve the integer constriants (only choco for now) 4. Solve the string
 * constriants with automata/sat/cvc 5. if step 4 gives unsat, and there is more
 * integer values that satisfy step 3, go to step 3 6. Translate the StringGraph
 * to the original symbolic strings.
 * 
...
public class SymbolicStringConstraintsGeneral {
	...
	private Vertex createVertex(StringExpression se) {
		boolean oldValue = PathCondition.isSolved();
		PathCondition.setSolved(false);
		String vertexName = se.getName();
		PathCondition.setSolved(oldValue);
	...
	/**
	 * Converts an expression to a subgraph, the subgraph will be added to the
	 * main graph later.
	 * 
	 * @param se
	 * @return
		...
		if (se instanceof StringConstant) {
			...
			case TRIM:
				...
				graphBefore.addEdge(v1, v2,
						new EdgeTrimEqual("EdgeTrimEqual_" + v1.getName() + "=" + temp.getName(), v1, v2));
				result = graphBefore;
				break;
			case SUBSTRING:
				...
				boolean oldState = PathCondition.isSolved();
				PathCondition.setSolved(false);
				v1 = graphBefore.findVertex(((StringExpression) temp.oprlist[0]).getName());
				PathCondition.setSolved(oldState);
				if (temp.oprlist[1] instanceof IntegerConstant
						&& (temp.oprlist.length == 2 || temp.oprlist[2] instanceof IntegerConstant)) {
					...
					if (temp.oprlist.length == 3) {
						...
						// println ("[convertToGraph, SUBSTRING] a1 = " + a1 +
						// ", a2 = " + a2);
						graphBefore.addEdge(v1, v2, new EdgeSubstring2Equal(
								"EdgeSubstring2Equal_" + v1.getName() + "_" + v2.getName() + "_(" + a2 + "," + a1 + ")",
								a2, a1, v1, v2));
					} else {
						v2 = createVertex(temp);
						global_spc.getNpc()._addDet(Comparator.EQ, v2.getSymbolicLength(),
								v1.getSymbolicLength()._minus(a1));
						graphBefore.addEdge(v1, v2,
								new EdgeSubstring1Equal(
										"EdgeSubstring1Equal_" + v1.getName() + "_" + v2.getName() + "_(" + a1 + ")",
										a1, v1, v2));
					}
					...
					global_spc.getNpc()._addDet(Comparator.EQ, v2.getSymbolicLength(),
							v1.getSymbolicLength()._minus(ie));
					graphBefore.addEdge(v1, v2, new EdgeSubstring1Equal(
							"EdgeSubstring1Equal_" + v1.getName() + "_" + v2.getName() + "_(" + ie.toString() + ")", ie,
							v1, v2));

				} else {
					// System.err.println ("Substring with symbolic integers not
					// yet supported");
					if (temp.oprlist[1] instanceof IntegerExpression && temp.oprlist.length == 3
							&& temp.oprlist[2] instanceof IntegerConstant) {
						v2 = createVertex(temp);
						...
						graphBefore.addEdge(v1, v2, new EdgeSubstring2Equal("EdgeSubstring2Equal_" + v1.getName() + "_"
								+ v2.getName() + "_(" + ie_a2 + "," + a1 + ")", a1, ie_a2, v1, v2));
					} else {
			...
			case REPLACE:
				/*
				 * graphLeft = convertToGraph((StringExpression)
				 * temp.oprlist[1]); graphRight = convertToGraph
				 * ((StringExpression) temp.oprlist[2]);
				 * result.mergeIn(graphLeft); result.mergeIn(graphRight);
				 */
				graphBefore = convertToGraph((StringExpression) temp.oprlist[0]);
				...
				e = new EdgeReplaceCharChar(
						"EdgeReplaceCharChar_" + v1.getName() + "_" + v2.getName() + "_(" + s1 + "," + s2 + ")", v1, v2,
						s2.solution().charAt(0), s1.solution().charAt(0));
				result.addEdge(v1, v2, e);
				break;
			case VALUEOF:
				/*
				 * I have to restrict myself here just because of DNF->CNF
				 * blowup
				 */
				// println("In valueof");
				IntegerExpression ie = (IntegerExpression) temp.oprlist[0];
				boolean oldSetting = PathCondition.isSolved();
				PathCondition.setSolved(false);
				v1 = new Vertex(temp.getName(), symbolicIntegerGenerator);
				PathCondition.setSolved(oldSetting);
				result.addVertex(v1);

				// Feeble attempt at log

				/*
				 * Constraint temp1 = new
				 * LinearIntegerConstraint(v1.getSymbolicLength(),
				 * Comparator.EQ, new IntegerConstant(1)); Constraint temp2 =
				 * new LinearIntegerConstraint(ie, Comparator.GT, new
				 * IntegerConstant(-10)); Constraint temp3 = new
				 * LinearIntegerConstraint(ie, Comparator.LT, new
				 * IntegerConstant(10)); temp1.and = temp2; temp2.and = temp3;
				 * 
				...
				for (int i = 1; i <= max - 1; i++) {
					...
					lolic.addToList(
							new LinearIntegerConstraint(v1.getSymbolicLength(), Comparator.LE, new IntegerConstant(i)));
					if (i == max - 1) {
						// Don't add anything
					} else {
						lolic.addToList(new LinearIntegerConstraint(ie, Comparator.GE,
								new IntegerConstant((int) Math.pow(10, i))));
					...
					if (i == 1) {
						...
						lolic.addToList(new LinearIntegerConstraint(ie, Comparator.LE,
								new IntegerConstant(-1 * ((int) Math.pow(10, i - 1)))));
					}
					global_spc.getNpc()._addDet(lolic);
				}
				...
				for (int i = 2; i <= max; i++) {
					lolic = new LogicalORLinearIntegerConstraints();
					lolic.addToList(
							new LinearIntegerConstraint(v1.getSymbolicLength(), Comparator.GE, new IntegerConstant(i)));
					lolic.addToList(new LinearIntegerConstraint(ie, Comparator.LT,
							new IntegerConstant((int) Math.pow(10, i - 1))));
					global_spc.getNpc()._addDet(lolic);

					lolic = new LogicalORLinearIntegerConstraints();
					lolic.addToList(
							new LinearIntegerConstraint(v1.getSymbolicLength(), Comparator.GE, new IntegerConstant(i)));
					lolic.addToList(new LinearIntegerConstraint(ie, Comparator.GT,
							new IntegerConstant(-1 * ((int) Math.pow(10, i - 1)))));
					global_spc.getNpc()._addDet(lolic);
				}
				...
				break;
			default:
				// println ("[WARNING] [convertToAutomaton] Did not understand "
				// + temp.op);
			}
		}
		return result;
	...
	/**
	 * Main entry point, solves (not only tests satisfiability) the given path
	 * condition
	 * 
	 * @param pc
	 * @return
	...
	private boolean inner_isSatisfiable(StringPathCondition pc) {
		...
		try {
			...
			/*
			 * Convert each clause in the path condition to a subgraph, and add
			 * it to the global_graph
			 */
			if (sc != null) {
			...
			/*
			 * Walk through integer constraints and convert each constraint to a
			 * subgraph and add it to the global_graph
			 */

			Constraint constraint = pc.getNpc().getHeader();
			// println ("[isSatisfiable] Int cons given:" + pc.npc.header);
			while (constraint != null) {
				processIntegerConstraint(constraint.getLeft(), constraint.getComparator(), constraint.getRight(),
						constraint);
				processIntegerConstraint(constraint.getRight(), constraint.getComparator(), constraint.getLeft(),
						constraint);
			...
			// boolean resultOfPp =
			// PreProcessGraphDisabled.preprocess(global_graph, pc.npc);
			// System.out.println("Number of edges after preprocessing: " +
			// global_graph.getEdges().size());
			/* check if there was a timeout */
			checkTimeOut();
			if (!resultOfPp) {
			 ...
			 * options are exhausted or a satisfiable solution has turned up
			 */
			...
			try {
				if (solver.equals(SAT)) {
					...
					boolean pcSolved = PathCondition.isSolved();
					// start with z3, and swap to automata if something goes
					// wrong
					try {
						decisionProcedure = TranslateToZ3.isSat(global_graph, pc.getNpc());
					} catch (Exception e) {
					...
					try {
						if (decisionProcedure == false) {
							...
							pc.setNpc(cloneNpc);
							PathCondition.setSolved(pcSolved);
				...
				if (!(e instanceof EdgeConcat)) {
					...
					if (represents != null) {
						for (StringSymbolic ss : represents) {
							...
							ss.setSolution(temp.getSolution());
							if (!setOfSolution.contains(ss)) {
								setOfSolution.add(ss);
							}
						}
					}
			...
			for (Vertex v : global_graph.getVertices()) {
				...
				for (Edge e : global_graph.getEdges()) {
					if (e.isHyper() && (e.getSources().get(0).equals(v) || e.getSources().get(1).equals(v)
							|| e.getDest().equals(v))) {
						inEdge = true;
			...
			StringPathCondition.setSolved(true);
			cancelTimer();
			if (SymbolicInstructionFactory.regressMode) {
	...
	/*
	 * Converts an integer-string constriant to a subgraph, which in turn is
	 * added to global_graph
	 */
	private void processIntegerConstraint(Expression e, Comparator comp, Expression other, Constraint origConstraint) {
		...
		if (e instanceof SymbolicCharAtInteger) {
			...
			// println ("[processIntegerConstraint] Found charAt constraint with
			// " + scai.se.getName());
			StringGraph sg = convertToGraph(scai.se);
			global_graph.mergeIn(sg);
			PathCondition.setSolved(true);
			/*
			 * if (!(scai.index instanceof IntegerConstant)) { throw new
			 * RuntimeException("OOPS! " + scai.index.toString()); } else {
			 * //println ("[processIntegerConstraint] scai.index.getClass(): " +
			 * scai.index.getClass()); }
			 */
			if (comp != null && comp == Comparator.NE && other instanceof IntegerConstant) {
				// println ("other: " + other);
				Vertex v1 = new Vertex("CharAt_" + scai.index.solution() + "_" + scai.solution(),
						String.valueOf((char) scai.solution()), true);
				Vertex v2 = global_graph.findVertex(scai.se.getName());
				global_graph.addEdge(v2, v1, new EdgeNotCharAt(
						"NotCharAt_" + scai.index.solution() + "_" + scai.solution(), v2, v1, scai.index, scai));
				// Necassery hack
				// TODO MAB: what is the function of this hack? it makes the
				// preprocessor return UNSAT on some valid constraints!
				origConstraint.setComparator(Comparator.EQ);
				global_spc.getNpc().setSolved(false);
			} else {
				Vertex v1 = new Vertex("CharAt_" + scai.index.solution() + "_" + scai.solution(),
						String.valueOf((char) scai.solution()), true);
				Vertex v2 = global_graph.findVertex(scai.se.getName());
				global_graph.addEdge(v2, v1, new EdgeCharAt("CharAt_" + scai.index.solution() + "_" + scai.solution(),
						v2, v1, scai.index, scai));
			}

		} else if (e instanceof SymbolicIndexOfInteger) {
			SymbolicIndexOfInteger sioi = (SymbolicIndexOfInteger) e;
			// println ("[processIntegerConstraint] Found indexOf constraint
			// with " + sioi.getName());
			...
			global_graph.addEdge(v2, v1,
					new EdgeIndexOf("EdgeIndexOf_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			...
			global_graph.addEdge(v2, v1,
					new EdgeLastIndexOf("EdgeLastIndexOf_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			PathCondition.setSolved(true); // TODO: Review
		} else if (e instanceof SymbolicLastIndexOf2Integer) {
			SymbolicLastIndexOf2Integer sioi = (SymbolicLastIndexOf2Integer) e;
			// println ("[processIntegerConstraint] Found indexOf2 constraint
			// with " + sioi.getName() + " and min dist: " +
			// sioi.getMinIndex().solution());
			StringGraph expression = convertToGraph(sioi.getExpression());
			StringGraph source = convertToGraph(sioi.getSource());
			global_graph.mergeIn(expression);
			global_graph.mergeIn(source);
			...
			Vertex v2 = global_graph.findVertex(sioi.getSource().getName());
			global_graph.addEdge(v2, v1,
					new EdgeLastIndexOf2("EdgeIndexOf2_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			...
			// println ("[processIntegerConstraint] Found indexOf (char)
			// constraint with " + sioi.getName());
			StringGraph source = convertToGraph(sioi.getSource());
			...
			Vertex v2 = global_graph.findVertex(sioi.getSource().getName());
			global_graph.addEdge(v2, v1,
					new EdgeLastIndexOfChar2("EdgeIndexOfChar_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			...
			if (sioi.getExpression() instanceof IntegerConstant) {
				v1 = new Vertex("CHAR_" + sioi.getExpression().solution(),
						String.valueOf((char) sioi.getExpression().solution()), true);
			} else {
			...
			Vertex v2 = global_graph.findVertex(sioi.getSource().getName());
			global_graph.addEdge(v2, v1,
					new EdgeIndexOfChar("EdgeIndexOfChar_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			PathCondition.setSolved(true); // TODO: Review
			...
			Vertex v2 = global_graph.findVertex(sioi.source.getName());
			global_graph.addEdge(v2, v1,
					new EdgeLastIndexOfChar("EdgeIndexOfChar_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			PathCondition.setSolved(true); // TODO: Review
			...
			Vertex v2 = global_graph.findVertex(sioi.source.getName());
			global_graph.addEdge(v2, v1,
					new EdgeIndexOfChar2("EdgeIndexOfChar_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			processIntegerConstraint(sioi.getMinDist(), null, null, null);
			PathCondition.setSolved(true); // TODO: Review
			...
			Vertex v2 = global_graph.findVertex(sioi.source.getName());
			global_graph.addEdge(v2, v1,
					new EdgeIndexOf2("EdgeIndexOf2_" + v2.getName() + "_" + v1.getName(), v2, v1, sioi));
			processIntegerConstraint(sioi.getMinIndex(), null, null, null);
			PathCondition.setSolved(true); // TODO: Review

		} else if (e instanceof SymbolicLengthInteger) {
			SymbolicLengthInteger sli = (SymbolicLengthInteger) e;
			// println ("[processIntegerConstraint] Found length constraint with
			// " + sli.getName());
		...
		/*
		 * else { if (e != null) { //println
		 * ("[processIntegerConstraint] Ignoring: " + e.getClass()); } }
		 */
	}
	...
	private boolean process(StringConstraint sc) {
		...
		case STARTSWITH:
			...
			v1 = global_graph.findVertex(se_left.getName());
			v2 = global_graph.findVertex(se_right.getName());
			global_graph.addEdge(v1, v2,
					new EdgeStartsWith("EdgeStartsWith_" + v1.getName() + "=" + v2.getName(), v1, v2));
			break;
		case NOTSTARTSWITH:
			// println ("[process, NOTSTARTSWITH] se_left: " +
			// se_left.toString());
			// println ("[process, NOTSTARTSWITH] se_right: " +
			// se_right.toString());
			leftGraph = convertToGraph(se_left);
			// println ("[process, NOTSTARTSWITH] left Graph: " +
			// leftGraph.toDot() + leftGraph.getVertices());
			rightGraph = convertToGraph(se_right);
			// println ("[process, NOTSTARTSWITH] right Graph: " +
			// rightGraph.toDot() + rightGraph.getVertices());
			global_graph.mergeIn(leftGraph);
			global_graph.mergeIn(rightGraph);
			// println ("[process, NOTSTARTSWITH] Trying to find: " +
			// se_left.getName());
			// println ("[process, NOTSTARTSWITH] graph: " +
			// global_graph.toDot());

			v1 = global_graph.findVertex(se_left.getName());
			v2 = global_graph.findVertex(se_right.getName());
			// println ("[process, NOTSTARTSWITH] global_graph null? " +
			// (global_graph == null));
			// println ("[process, NOTSTARTSWITH] v1 null? " + (v1 == null));
			// println ("[process, NOTSTARTSWITH] v2 null? " + (v2 == null));
			global_graph.addEdge(v1, v2,
					new EdgeNotStartsWith("EdgeNotStartsWith_" + v1.getName() + "=" + v2.getName(), v1, v2));
			break;
		case ENDSWITH:
			leftGraph = convertToGraph(se_left);
			rightGraph = convertToGraph(se_right);
		...
		case NOTENDSWITH:
			...
			v2 = global_graph.findVertex(se_right.getName());
			global_graph.addEdge(v1, v2,
					new EdgeNotEndsWith("EdgeNotEndsWith_" + v1.getName() + "=" + v2.getName(), v1, v2));
			break;
		case CONTAINS:
			leftGraph = convertToGraph(se_left);
			// println("LEFT GRAPH: \n" + leftGraph.toDot());

	...
	private final static class Translator extends ConstraintExpressionVisitor {
		...
		public void postVisit(Constraint constraint) {
			...
			case EQ:
				...
				if (r instanceof CharAtVariable) {
					// We only handle the case where the other operand is
					// another charAt variable or an integer constant.
					if (l instanceof CharAtVariable) {
						...
						System.out.println(
								"SolverTranslator : unsupported charAt case - the other operand is not an integer constant or charAt term!");
		...
		public void postVisit(DerivedStringExpression expression) {
			...
			case REPLACEFIRST:
				...
				break;
			/*
			 * case TOLOWERCASE: r = stack.pop(); l = stack.pop(); n =
			 * stack.pop(); stack.push(new
			 * Operation(Operation.Operator.TOLOWERCASE, n,l, r)); break;
			 */
			case SUBSTRING:
				r = stack.pop();
				l = stack.pop();
				...
				break;
			case VALUEOF: // This assumes that Operation.Operator.VALUEOF was
							// incorrectly 2 (see that file).
				l = stack.pop();
		...
		public void postVisit(StringConstant constant) {
			stack.push(new StringConstantGreen((String) constant.getValue()));
		}

		@Override
		public void postVisit(IntegerConstant constant) {
			stack.push(new IntConstant((int) constant.getValue()));
		}
		...
		public void postVisit(SymbolicInteger node) {
			if (node instanceof SymbolicLengthInteger) {
				...
				stack.push(new LengthVariable(node.getName(), (int) node.getMin(), (int) node.getMax(), parent_green));
				...
				stack.push(new CharAtVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						parent_green, index_green));
				...
				stack.push(new IndexOfVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green));
				...
				stack.push(new IndexOf2Variable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green, min_dist_green));
				...
				expr.accept(translator_internal);
				Expression expr_green = translator_internal.getExpression();
				stack.push(new IndexOfCharVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green));
				...
				min_dist.accept(translator_internal);
				Expression min_dist_green = translator_internal.getExpression();
				stack.push(new IndexOfChar2Variable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green, min_dist_green));
				...
				expr.accept(translator_internal);
				Expression expr_green = translator_internal.getExpression();
				stack.push(new LastIndexOfVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green));
				...
				min_dist.accept(translator_internal);
				Expression min_dist_green = translator_internal.getExpression();
				stack.push(new LastIndexOf2Variable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green, min_dist_green));
				...
				expr.accept(translator_internal);
				Expression expr_green = translator_internal.getExpression();
				stack.push(new LastIndexOfCharVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green));
				...
				min_dist.accept(translator_internal);
				Expression min_dist_green = translator_internal.getExpression();
				stack.push(new LastIndexOfChar2Variable(node.getName(), node, (int) node.getMin(), (int) node.getMax(),
						source_green, expr_green, min_dist_green));
			} else if (node instanceof SymbolicIndexOfChar2Integer) {
				SymbolicIndexOfChar2Integer node_c = (SymbolicIndexOfChar2Integer) node;
				StringExpression source = node_c.getSource();
				...
				stack.push(new IntVariable(node.getName(), node, (int) node.getMin(), (int) node.getMax()));
			}
		}
	}
}

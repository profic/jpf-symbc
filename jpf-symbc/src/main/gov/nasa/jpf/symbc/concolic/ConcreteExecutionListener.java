...
public class ConcreteExecutionListener extends PropertyListenerAdapter {
	...
	public enum type {
		INT, DOUBLE, FLOAT, BYTE, SHORT, LONG, BOOLEAN, CHAR
	}
	...
	public void instructionExecuted(VM vm, ThreadInfo currentThread, Instruction nextInstruction,
		...
		if (lastInsn != null && lastInsn instanceof JVMInvokeInstruction) {
			...
			if (foundAnote) {
				...
				FunctionExpression result = generateFunctionExpression(mi, (JVMInvokeInstruction) lastInsn, ti);
	...
	private boolean checkConcreteAnnotation(MethodInfo mi) {
		...
		for (int aIndex = 0; aIndex < ai.length; aIndex++) {
			...
			if (annotation.getName().equals("gov.nasa.jpf.symbc.Concrete")) {
				if (annotation.valueAsString().equalsIgnoreCase("true"))
					retVal = true;
				else
					retVal = false;
			} else if (annotation.getName().equals("gov.nasa.jpf.symbc.Partition")) {
	...
	private FunctionExpression generateFunctionExpression(MethodInfo mi, JVMInvokeInstruction ivk, ThreadInfo ti) {
		...
		for (int argIndex = 0; argIndex < size; argIndex++) {
			...
			if (attribute == null) {
				sym_args[argIndex] = this.generateConstant(types[argIndex], values[argIndex]);
			} else {
				sym_args[argIndex] = (Expression) attribute;
				if (localVars.length > argIndex)
					expressionMap.put(localVars[argIndex].getName(), sym_args[argIndex]);

			}
			args_type[argIndex] = checkArgumentTypes(types[argIndex]);
		}

		ArrayList<PathCondition> conditions = Partition.createConditions(partitions, expressionMap);

		FunctionExpression result = new FunctionExpression(mi.getClassName(), mi.getName(), args_type, sym_args,
				conditions);
	...
	private Expression generateConstant(String typeVal, Object value) {
		if (typeVal.equals("int")) {
			return new IntegerConstant(Integer.parseInt(value.toString()));
		} else if (typeVal.equals("double")) {
			return new RealConstant(Double.parseDouble(value.toString()));
		} else if (typeVal.equals("float")) {
			return new RealConstant(Float.parseFloat(value.toString()));
		} else if (typeVal.equals("long")) {
			return new IntegerConstant((int) Long.parseLong(value.toString()));
		} else if (typeVal.equals("short")) {
			return new IntegerConstant((int) Short.parseShort(value.toString()));
		} else if (typeVal.equals("boolean")) {

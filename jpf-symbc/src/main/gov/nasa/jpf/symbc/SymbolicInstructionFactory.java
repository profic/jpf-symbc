...
public class SymbolicInstructionFactory extends gov.nasa.jpf.jvm.bytecode.InstructionFactory {
	...
	public Instruction ifle(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFLE(targetPc) : new IFLE(targetPc)
				: super.ifle(targetPc));
	}

	public Instruction iflt(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFLT(targetPc) : new IFLT(targetPc)
				: super.iflt(targetPc));
	}

	public Instruction ifge(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFGE(targetPc) : new IFGE(targetPc)
				: super.ifge(targetPc));
	}

	public Instruction ifgt(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFGT(targetPc) : new IFGT(targetPc)
				: super.ifgt(targetPc));
	}

	public Instruction ifeq(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFEQ(targetPc) : new IFEQ(targetPc)
				: super.ifeq(targetPc));
	}

	public Instruction ifne(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IFNE(targetPc) : new IFNE(targetPc)
				: super.ifne(targetPc));
	}
	...
	public Instruction if_icmpge(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPGE(targetPc) : new IF_ICMPGE(targetPc)
				: super.if_icmpge(targetPc));
	}

	public Instruction if_icmpgt(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPGT(targetPc) : new IF_ICMPGT(targetPc)
				: super.if_icmpgt(targetPc));
	}

	public Instruction if_icmple(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPLE(targetPc) : new IF_ICMPLE(targetPc)
				: super.if_icmple(targetPc));
	}

	public Instruction if_icmplt(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPLT(targetPc) : new IF_ICMPLT(targetPc)
	...
	public Instruction if_icmpeq(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPEQ(targetPc) : new IF_ICMPEQ(targetPc)
				: super.if_icmpeq(targetPc));
	}

	public Instruction if_icmpne(int targetPc) {
		return (filter.isPassing(ci) ? (pcChoiceOptimization)
				? new gov.nasa.jpf.symbc.bytecode.optimization.IF_ICMPNE(targetPc) : new IF_ICMPNE(targetPc)
	...
	public Instruction fcmpg() {
		return (filter.isPassing(ci)
				? (pcChoiceOptimization) ? new gov.nasa.jpf.symbc.bytecode.optimization.FCMPG() : new FCMPG()
				: super.fcmpg());
	}

	public Instruction fcmpl() {
		return (filter.isPassing(ci)
				? (pcChoiceOptimization) ? new gov.nasa.jpf.symbc.bytecode.optimization.FCMPL() : new FCMPL()
				: super.fcmpl());
	}
	...
	public Instruction dcmpg() {
		return (filter.isPassing(ci)
				? (pcChoiceOptimization) ? new gov.nasa.jpf.symbc.bytecode.optimization.DCMPG() : new DCMPG()
				: super.dcmpg());
	}

	public Instruction dcmpl() {
		return (filter.isPassing(ci)
				? (pcChoiceOptimization) ? new gov.nasa.jpf.symbc.bytecode.optimization.DCMPL() : new DCMPL()
				: super.dcmpl());
	}
	...
	/*
	 * This is intended to serve as a catchall debug flag. If there's some debug
	 * printing/outputing, conditionally print using this flag.
	 */
	...
	/*
	 * If Green is enabled this solver will be used Later we just check if this
	 * is null to know if Green is enabled
	 */
	static public Green greenSolver = null;

	/*
	 * Allow user to set the bitvector length for Z3bitvector and potentially
	 * other bv-based solvers.
	 */
	static public int bvlength;

	/*
	 * Use floating point theory for reals in Z3 (or other solvers that might
	 * support this).
	 */
	...
	/*
	 * With this setting, pc choices are only added if multiple branches are
	 * feasible
	 */
	...
	private void setupGreen(Config conf) {
		...
		// fix to make sure when Green is used there is no NPE when poking at
		// dp[0] in some bytecodes
		dp = new String[] { "green" };
	}
	...
	public SymbolicInstructionFactory(Config conf) {
		...
		MinMax.loadBounds(conf);
		/*
		 * no longer required here, now read in MinMax, see line above
		 * 
		 * String[] intmin, intmax, realmin, realmax, dontcare; intmin =
		 * conf.getStringArray("symbolic.minint"); intmax =
		 * conf.getStringArray("symbolic.maxint"); realmin =
		 * conf.getStringArray("symbolic.minreal"); realmax =
		 * conf.getStringArray("symbolic.maxreal"); dontcare =
		 * conf.getStringArray("symbolic.undefined");
		 * 
		 * if (intmin != null && intmin[0] != null) MinMax.MININT = new
		 * Integer(intmin[0]); if (intmax != null && intmax[0] != null)
		 * MinMax.MAXINT = new Integer(intmax[0]); if (realmin != null &&
		 * realmin[0] != null) MinMax.MINDOUBLE = new Double(realmin[0]); if
		 * (realmax != null && realmax[0] != null) MinMax.MAXDOUBLE = new
		 * Double(realmax[0]); if (dontcare != null && dontcare[0] != null) {
		 ...
		 * if((SymbolicInteger.UNDEFINED >= MinMax.MININT &&
		 * SymbolicInteger.UNDEFINED <= MinMax.MAXINT) &&
		 * (SymbolicInteger.UNDEFINED >= MinMax.MINDOUBLE &&
		 * SymbolicInteger.UNDEFINED <= MinMax.MAXDOUBLE)) System.err.

...
/**
 * base class for JPF unit tests. TestJPF mostly includes JPF invocations that
 * check for occurrence or absence of certain execution results
 *
 * This class can be used in two modes:
 *
 * <ol>
 * <li>wrapping a number of related tests for different SuTs into one class
 * (suite) that calls the various JPF runners with complete argument lists (as
 * in JPF.main(String[]args))</li>
 *
 * <li>derive a class from TestJPF that uses the "..This" methods, which in turn
 * use reflection to automatically append the test class and method to the
 ...
 * "CastTest" -> "Cast"</li>
 *
 * <li>the SuT method has to have the same name as the @Test method that invokes
 * JPF, e.g. "CastTest {.. @Test void testArrayCast() ..}" -> "Cast {.. void
 * testArrayCast()..}</li>
 * </ol>
 */
public abstract class TestJPF implements JPFShell {
	...
	@FilterField
	protected static boolean runDirectly; // don't run test methods through JPF,
											// invoke it directly
	@FilterField
	protected static boolean stopOnFailure; // stop as soon as we encounter a
		...
		if (globalConf != null) {
			...
			if (testKeys.length > 0) {
				for (String key : testKeys) {
					...
					// <2do> this is a hack to avoid the problem of not being
					// able to store
					// empty/nil/null values in the global config (they are
					// removed during global config init)
	 ...
	 * @param sutPackage
	 *            optional SuT package name (without ending '.', null os
	 *            SAME_PACKAGE means same package, "" or UNNAMED_PACKAGE means
	 *            unnamed package)
	...
	protected static String getSutClassName(String testClassName, String sutPackage) {
		...
		if (i >= 0) { // testclass has a package
			...
			if (sutPackage == null || sutPackage.length() == 0) { // use same
																	// package
				// nothing to do
	...
	// runTestsOfThisClass()) since main() doesn't get called if this is
	// executed
	...
	/**
	 * to be used from default ctor of derived class if the SuT is in a
	 * different package
	 * 
	 * @param sutClassName
	 *            the qualified SuT class name to be checked by JPF
	 */
	...
	protected static void getOptions(String[] args) {
		...
		// hideSummary and stopOnFailure only make sense as global options
		// anyways
	...
	protected static List<Method> getMatchingMethods(Class<? extends TestJPF> testCls, int setModifiers,
			int unsetModifiers, String[] annotationNames) {
	...
	protected static boolean isMatchingMethod(Method m, int setModifiers, int unsetModifiers,
	...
	protected static List<Method> getContextMethods(Class<? extends TestJPF> testCls, int setModifiers,
			int unsetModifiers, String annotation) {
					...
					try {
						...
						if (isMatchingMethod(m, Modifier.PUBLIC, Modifier.STATIC, null /* testAnnotations */ )) {
							...
							throw new RuntimeException(
									"test method must be @Test annotated public instance method without arguments: "
											+ test);
						}

					} catch (NoSuchMethodException x) {
						throw new RuntimeException("method: " + test + "() not in test class: " + testCls.getName(), x);
					}
				}
			}
	...
	public static void runTests(Class<? extends TestJPF> testCls, String... args) {
		...
		try {
			...
			// check if we have JUnit style housekeeping methods (initialization
			// and
			...
			// --- those exceptions are unexpected and represent unrecoverable
			// test harness errors
		} catch (InvocationTargetException x) {
			Throwable cause = x.getCause();
			cause.printStackTrace();
		...
		} catch (IllegalAccessException x) { // can't happen if getTestMethods()
												// worked
			nErrors++;
			reportTestFinished("TEST ERROR: default constructor or test method not public: " + testMethodName);
		} catch (IllegalArgumentException x) { // can't happen if
												// getTestMethods() worked
			nErrors++;
	...
	/**
	 * NOTE: this needs to be called from the concrete test class, typically
	 * from its main() method, otherwise we don't know the name of the class we
	 * have to pass to JPF
	 */
	...
	/**
	 * needs to be broken up into two methods for cases that do additional JPF
	 * initialization (jpf-inspector)
	 *
	 * this is called from the various verifyX() methods (i.e. host VM) to start
	 * JPF, it is never executed under JPF
	 */
	...
	public void start(String[] testMethods) {
		Class<? extends TestJPF> testClass = getClass(); // this is an instance
															// method
		runTests(testClass, testMethods);
	}
	...
	/**
	 * NOTE: this uses the exception class name because it might be an exception
	 * type that is only known to JPF (i.e. not in the native classpath)
	 *
	 * @param xClassName
	 *            name of the exception base type that is expected
	 * @param details
	 *            detail message of the expected exception
	 * @param args
	 *            JPF arguments
	 */
				...
				if (details != null) {
					...
					if (gotDetails == null) {
						...
						if (!gotDetails.endsWith(details)) {
							fail("JPF caught the right exception but the details were wrong: " + gotDetails
									+ ", expected: " + details);
						}
					}
				}
	...
	/**
	 * run JPF expecting it to throw an exception NOTE - xClassName needs to be
	 * the concrete exception, not a super class
	 * 
	 * @param args
	 *            JPF main() arguments
	 */
	 ...
	 *            JPF main() arguments
	 */
	protected JPF propertyViolation(StackTraceElement testMethod, Class<? extends Property> propertyCls,
			String... args) {
		JPF jpf = null;
	...
	public static void assertEquals(double expected, double actual, double delta) {
		assertEquals(
				"Math.abs(expected - actual) > delta : " + "Math.abs(" + expected + " - " + actual + ") > " + delta,
